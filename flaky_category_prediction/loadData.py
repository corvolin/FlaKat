...
def loadRaw(csvLocation, onlyFlaky):
    ...
    dataCSV['category'].replace(np.nan, 'NF', inplace=True)
    
    # only pick rows with the desired flaky category
    if onlyFlaky:
        dataCSV = dataCSV[(dataCSV['category'] == 'ID') |(dataCSV['category'] == 'OD') | (dataCSV['category'] == 'OD-Vic') | (dataCSV['category'] == 'OD-Brit') | (dataCSV['category'] == 'NOD') ]
        dataCSV['category'] = dataCSV['category'].replace('OD-Vic','OD')
        dataCSV['category'] = dataCSV['category'].replace('OD-Brit','OD')
    else:
...
def loadCode2vecEmbedding(csvLocation, features, categoryDf, outputFeatures, onlyFlaky):
    ...
    # only pick rows with the desired flaky category
    if onlyFlaky:
        formatedDf = formatedDf[(formatedDf['category'] == 'ID')|(formatedDf['category'] == 'OD') | (formatedDf['category'] == 'NOD') ]
    else:
